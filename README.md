...
- 平日（月曜〜金曜・祝日除く）に1分以上の実勤務がある日: **みなし残業 1.5 時間**（実勤務の長短に依存しない）
- **平日の法定外（残業）**: `max(1:30, 実勤務 − 8:00)`（月45時間判定にも同値を使用）
- **土曜・祝日（日曜以外）**: 実勤務全量を法定外（休除）。みなしなし
- **日曜**: 法定休日労働（45時間判定から除外）
- **みなし法定外**: 法定外（休除）＋法定休日労働
- **深夜**: 22:00〜翌5:00の実勤務（45時間判定には二重加算しない）
- 通常勤務体系ではみなし関連は計算・表示しない

警告（裁量時）: 月45時間超過、単月100時間未満までの残り（上限99:59）、80時間警告、2〜6か月平均80時間超。
